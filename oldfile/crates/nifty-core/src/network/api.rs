...
fn get_agent(proxy_url: Option<&str>) -> Result<ureq::Agent, Box<dyn std::error::Error>> {
...
pub fn fetch_nifty_story(
  url: &str,
  proxy_url: Option<&str>,
) -> Result<Story, Box<dyn std::error::Error>> {
  let agent = get_agent(proxy_url)?;
