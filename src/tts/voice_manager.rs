...
impl VoiceManager {
    ...
    pub fn remove_voice(&mut self, name: &str) -> Result<(), String> {
        ...
        if filepath.exists() {
            fs::remove_file(&filepath)
                .map_err(|e| format!("Failed to remove voice file: {}", e))?;
            self.cached_states.remove(name);
            Ok(())
        } else {
    ...
    pub fn get_voice_state(&mut self, voice_name: &str, engine: &mut crate::tts::TTSEngine) -> Result<VoiceState, String> {
        // Check cache first
        ...
        let state = if voice_name == "Default" {
            // Use default voice state (this would need to be implemented in pocket-tts)
            // For now, return an error - we'll need to see what pocket-tts provides
            return Err("Default voice not implemented yet".to_string());
        } else {
            // Load from file
        ...
        // Cache the state
        self.cached_states.insert(voice_name.to_string(), state.clone());
