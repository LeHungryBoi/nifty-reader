...
pub fn BrowseView(props: BrowseViewProps) -> Element {
    ...
    let stories_in_cat: Vec<_> = stories.iter().filter(|s| {
    ...
    let filtered_stories: Vec<_> = stories.iter().filter(|s| {
        let cat_match = *selected_category.read() == "All" || s.categories.contains(&*selected_category.read());
        let sub_match = *selected_subcategory.read() == "All" || s.subcategories.contains(&*selected_subcategory.read());
        cat_match && sub_match
    }).collect();
