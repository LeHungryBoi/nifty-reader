# pocket-tts Integration Guide for Nifty Reader

Source: https://github.com/babybirdprd/pocket-tts

## What it is
Pure Rust, CPU-only TTS using Candle (HuggingFace tensor framework). NOT a system TTS wrapper.
It is a neural TTS model (Kyutai's Pocket TTS architecture) — generates WAV audio from text.
Models are downloaded from HuggingFace during build time and cached locally.
...
**Important:** Model downloads have been separated from the build process to avoid compilation blocking.
...
The download script fetches three files to `models/` directory:
...
## Predefined Voices (no reference WAV needed)
...
## Model Loading in Nifty Reader

The project uses a singleton pattern with `OnceLock` for efficient model caching:
...
See `.codebuddy/skills/huggingface-model-hash/SKILL.md` for detailed workflow.
